...
Header ``<HHHHI``: structure version (1), zone bits (8 -> 256x256 samples per
zone), zones X, zones Z, map version (10). Samples are little-endian uint16
whose low 13 bits hold the height, stored zone by zone (row-major zones, each
zone row-major, south first). Stock authoring stays at or below 0x0FFF.
"""
...
def read_hg2(path: os.PathLike | str) -> tuple[HG2Header, np.ndarray]:
    ...
    if len(payload) != expected_bytes:
        raise ValueError(
            f"HG2 payload size mismatch (sample count mismatch): expected {expected_bytes} bytes, "
            f"found {len(payload)}"
        )
...
# ---------------------------------------------------------------------------
# Object API (formerly HeightmapGen's HG2Map)
# ---------------------------------------------------------------------------
...
class HG2Map:
    ...
    def read(cls, path: os.PathLike | str) -> "HG2Map":
        ...
        return cls(heights, header.zones_x, header.zones_z, header.zone_bits,
                   header.structure_version, header.map_version)

    def write(self, path: os.PathLike | str) -> None:
        self.validate_storage()
        write_hg2(path, self.heights, self.zones_x, self.zones_z, self.zone_bits,
                  self.structure_version, self.map_version)
    ...
    def write_png16(self, path: os.PathLike | str) -> None:
        ...
        self.validate_storage()
        Image.fromarray(hg2_to_png16_array(self.heights)).save(path)  # uint16 -> "I;16"
